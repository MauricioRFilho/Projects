...
function CookieCache() {
  ...
  this.add = function (cookie) {
    var domain = 'name: ' + cookie.name + '   value: ' + cookie.value;
    if (!this.cookies_[domain]) {
      this.cookies_[domain] = [];
    }
    this.cookies_[domain].push(domain);
  };


  this.remove = function (cookie) {
    var domain = cookie.value;
    if (this.cookies_[domain]) {
      var i = 0;
...
function reloadCookieTable() {
  ...
  domains.forEach(function (domain) {
    ...
    // var value = cache.getCookies(value)
    // var value =value.getCookies(domain)
    var row = table.insertRow(-1);
    // console.log(domain)
    // row.insertCell(-1).innerText = valueCo;
